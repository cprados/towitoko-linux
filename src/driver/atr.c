...
atr_d_table[16] = 
{ 
  0, 1, 2, 4, 8, 16, 0, 0, 0, 0, 0.5, 0.25, 125, 0.0625, 0.03125, 0.015625 
};
...
static bool
ATR_GetNextByte (IO_Serial * io, unsigned timeout, BYTE * b, bool invert);
...
int
ATR_InitFromStream (ATR * atr, IO_Serial * io, unsigned timeout)
{
  BYTE TDi;
  ...
  while (TRUE)
      ...
      if ((TDi | 0xEF) == 0xFF)
	{
	  pointer++;
	  if (!ATR_GetNextByte
	      (io, timeout, &(atr->ib[pn][ATR_INTERFACE_BYTE_TA].value),
	       invert))
	    return ATR_MALFORMED;
	  atr->ib[pn][ATR_INTERFACE_BYTE_TA].present = TRUE;
	}
      ...
      if ((TDi | 0xDF) == 0xFF)
	{
	  pointer++;
	  if (!ATR_GetNextByte
	      (io, timeout, &(atr->ib[pn][ATR_INTERFACE_BYTE_TB].value),
	       invert))
	    return ATR_MALFORMED;
	  atr->ib[pn][ATR_INTERFACE_BYTE_TB].present = TRUE;
	}
      ...
      if ((TDi | 0xBF) == 0xFF)
	{
	  pointer++;
	  if (!ATR_GetNextByte
	      (io, timeout, &(atr->ib[pn][ATR_INTERFACE_BYTE_TC].value),
	       invert))
	    return ATR_MALFORMED;
	  atr->ib[pn][ATR_INTERFACE_BYTE_TC].present = TRUE;
	}
      ...
      if ((TDi | 0x7F) == 0xFF)
	{
	  pointer++;
	  if (!ATR_GetNextByte
	      (io, timeout, &(atr->ib[pn][ATR_INTERFACE_BYTE_TD].value),
	       invert))
	    return ATR_MALFORMED;
...
static bool
ATR_GetNextByte (IO_Serial * io, unsigned timeout, BYTE * byte, bool invert)
{
  bool ret;
